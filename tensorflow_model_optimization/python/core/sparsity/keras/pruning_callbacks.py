...
class LTHPruningStep(callbacks.Callback):
    """Keras callback which updates pruning wrappers with the optimizer step.
    ...
    ```python
    model.fit(x, y,
        callbacks=[LTHPruningStep()])
    ```
    """

    def __init__(self, ckpt_path, reset_itr=0, end_step=-1):
        super(LTHPruningStep, self).__init__()
        self.prunable_layers = []
        self.ckpt_path = ckpt_path
        # our "initialization" can be reset to any generic state in training
        self.reset_step = reset_step
        self.end_step = end_step
        # weights to re-initialize to
        self.lottery_tix = None
    ...
    def on_train_begin(self, logs=None):
        ...
        # save model parameters at correct step
        if self.step == self.reset_step:
            self.lottery_tix = self.model.get_weights()

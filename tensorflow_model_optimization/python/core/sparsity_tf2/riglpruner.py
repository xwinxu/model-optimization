...
class RiGLPruner(pruner.LowMagnitudePruner):
  """
  ...
  def __init__(self,
      ...
      initializer=sparse_utils.uniform,
  ):
    ...
    Args:
      ...
      initializer: the initial sparsity distribution for each layer of the network.
    """
    ...
    self.initial_sparsity = pruning_schedule.get_config()['config']['target_sparsity']
    self.initializer = initializer
  
  def create_slots(self, optimizer, var):
    base_dtype = var.dtype
    # TODO(xwinxu): support ER and ERK initialization, implement bernouilli initializer
    optimizer.add_slot(var, 'mask', initializer=self.initializer)
  ...
  def postprocess_weights(self, optimizer, var, grad):
    ...
    """
    mask = optimizer.get_slot(var, 'mask')                                                                                                       self.grow
    self.drop
    self.grow
